...
const OUT = resolve(
  ...
  "../../api/sigil/testdata/corpus.json",
);
 ...
 */
if (
  !engine.startsWith(PINNED + "/") &&
  !process.argv.includes("--allow-engine-change")
) {
  console.error(
...
for (let i = 0; i < 300; i++) {
  ...
  rows.push({
    ...
    clamped: c.telemetry,
    violations: c.violations.slice().sort(),
    hash,
...
console.log(
  ...
  rows[0].hash,
  "| clamped[0]",
  JSON.stringify(rows[0].clamped),
);
const spread = rows.reduce<Record<string, number>>(
  (a, r) => ({ ...a, [r.rarity]: (a[r.rarity] ?? 0) + 1 }),
  {},
);
