...
export function Certificate({ pour, grain = true }: Props) {
  ...
  if (!RARITY_PRESENTATION[rarity]) {
    console.error(
      `certificate: unknown rarity ${JSON.stringify(rarity)}, falling back to Common`,
    );
    rarity = "Common";
  }
  ...
  const rows: [string, string][] = [
    ...
    ["Substance", substance],
    // The score alongside the band, because the band alone hides the distance
    // between a pour that just cleared a threshold and one that ran away with it.
    // Rounded: the server stores it as a float but computes it in integers.
    ["Classification", `${rarity} · ${Math.round(pour.rarityScore)}`],
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 33,
          pointerEvents: "none",
          border: `1px solid ${COLOR.ruleMid}`,
        }}
      />
          ...
          >
            <span
              style={{
                ...
                height: 1,
                background:
                  "linear-gradient(90deg,transparent,rgba(201,146,46,.55))",
              }}
            />
            <span
              style={{
            ...
            <span
              style={{
                ...
                height: 1,
                background:
                  "linear-gradient(90deg,rgba(201,146,46,.55),transparent)",
              }}
            />
          </div>
        </div>
